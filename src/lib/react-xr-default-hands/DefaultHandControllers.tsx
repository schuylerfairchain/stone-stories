        ...
        if (model && !model?.loading) {
          ...
          actions.forEach((x) =>
            handActions.current[c.inputSource.handedness]!.splice(
              handActions.current[c.inputSource.handedness]!.indexOf(x),
              1,
            ),
          );
    ...
    <>
      {process.env.NODE_ENV === 'development' &&
        controllers.map((c, index) =>
          models?.current[c.inputSource.handedness] ? (
            <Axes key={index} controller={c} model={models!.current[c.inputSource.handedness]!} />
          ) : null,
        )}
    </>
  );

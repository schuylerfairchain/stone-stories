...
export function Axes({ controller, model, useGuides = false }: Props) {
  useFrame(() => {
    ...
    const indexKnuckle = model!.bones.find(
      (bone) => (bone as any).jointName === 'index-finger-metacarpal',
    )! as Object3D;
    const pinkyKnuckle = model!.bones.find(
      (bone) => (bone as any).jointName === 'pinky-finger-metacarpal',
    )! as Object3D;
    ...
    <group>
      {useGuides && (
        <>
          <mesh
            ref={thumbTipRef}
            geometry={new SphereGeometry(0.0000001)}
            material={new MeshBasicMaterial({ color: 'blue' })}
          />
          <mesh
            ref={thumbJointRef}
            geometry={new SphereGeometry(0.000001)}
            material={new MeshBasicMaterial({ color: 'brown' })}
          />
          <mesh
            ref={thumbTipCollidingRef}
            geometry={new SphereGeometry(0.000001)}
            material={new MeshBasicMaterial({ color: 'orange' })}
          />
          <mesh
            ref={indexTipRef}
            geometry={new SphereGeometry(0.0000005)}
            material={new MeshBasicMaterial({ color: 'blue' })}
          />
          <mesh
            ref={indexJointRef}
          ...
          <mesh
            ref={positionRef}
            geometry={new SphereGeometry(0.000000005)}
